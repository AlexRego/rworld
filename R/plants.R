...
setup.plants <- function(names=NULL){
  ...
  info <- list(repro=repro,survive=survive,comp.mat=comp.mat,names=names)
  return(info)
}

info <- setup.plants(names=NULL)

#' Setting up plant information
#'
#' @param terrain Give original 2D terrain matrix
#' @param timesteps Inputs how many timesteps will be present in ecosystem
#' @return returns an array that matches matrix at some number of timesteps

# set up ecosystem
eco.gen <- function(terrain, timesteps){
